        ...
        {              
          this.state.restaurants.map(function(listing){
            ...
            <TouchableOpacity style={styles.results}>
              <Image source={{ uri: listing.image_url }} style={{width: 80, height: 80, alignItems: 'flex-end'}} />
              <View style={{flexDirection: 'column', justifyContent: 'center'}}>
                <Text style={{fontWeight: 'bold'}}>{listing.name}</Text>
                <Text>Rating: {listing.rating}</Text>
                <Text>Phone: {listing.phone}</Text>
                <Text>Distance: {listing.distance} ft.</Text>
