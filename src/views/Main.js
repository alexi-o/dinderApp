...
class Main extends Component {
  ...
  "region": {
    "center": {
      "latitude": 39.70573111106199,
      "longitude": -104.9662971496582
    }
  }
}
  setPosition(e){
          ...
          this.state.restaurants.map(function(listing){
            ...
            >
              ...
              <View style={{ margin: 10, flexDirection: 'column', justifyContent: 'center'}}>
                <Text style={{fontWeight: 'bold', fontSize: 20}}>{listing.name}</Text>
                <Text style={{fontSize: 15}}>Rating: {listing.rating}</Text>
                <Text style={{fontSize: 15}}>Phone: {listing.display_phone}</Text>
                <Text style={{fontSize: 15}}>Distance: {listing.distance} ft.</Text>
              </View>
